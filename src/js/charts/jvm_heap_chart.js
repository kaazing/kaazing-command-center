...
/**
 * Chart to show current JVM heap usage for a given gateway's JVM.
 *
 * @module jvm-heap-chart
 */
YUI.add('jvm-heap-chart', function(Y) {
    ...
    /**
     * The View for the JVM 'current heap percentage' chart
     *
     * @class JvmHeapChart
    ...
    Y.JvmHeapChart = Y.Base.create(
        ...
        [], 
        {
            initializer: function() {
                ...
                // We're going to listen for JVM 'update' events ONLY.
                ...
                $this.on(Y.JvmModel.prototype.UPDATE_EVENT, 
                            $this.onJvmUpdate,
                            $this);
            },
            ...
            onJvmUpdate: function(ev) {
                ...
                var jvmModel = ev.model;
                var gatewayModel = jvmModel.get('gatewayModel');
                ...
                // Get the indexes of the values we need to read from the JVM data
                var memHeapUsedIndex = this.get('memHeapUsedIndex'); 
                var readTimeIndex = this.get('readTimeIndex');

                if (memHeapUsedIndex == null) {
                    readTimeIndex = gatewayModel.summaryAttributeIndex('jvm', 'readTime');
                    memHeapUsedIndex = gatewayModel.summaryAttributeIndex('jvm', 'memHeapUsed');
                    this.set('readTimeIndex', readTimeIndex);
                    this.set('memHeapUsedIndex', memHeapUsedIndex);
                }

                var newTime = ev.data[readTimeIndex];
                var newValue = ev.data[memHeapUsedIndex];
        ...
        }, 
        {
            ATTRS: {
                ...
                // of [time, value] pairs, the relevant JVM 'memHeapUsed'
                // property value for that gateway at that specific time.
                dataItemValues: {
                    value: {}
                },

                // the index in the summary data of the 'memHeapUsed' property.
