...
/**
 * The view for gateway/cluster keystore objects.
 *
 * @module config-security-keystore-view
 */
YUI.add('config-security-keystore-view', function(Y) {
    ...
    var Lang = Y.Lang,
    NAME = 'configSecurityKeystoreView',
    ...
    /**
     * The View for displaying information about the keystore(s) used by a gateway/cluster. 
     * As with Services, we 'coalesce' where the keystores match across gateways
     *
     * @class ConfigSecurityKeystoreView
     * @extends Y.KaazingView
     * @uses xxxxx
     * @constructor
     * @cfg {object} configuration attributes
     */
    Y.ConfigSecurityKeystoreView = Y.Base.create(
        NAME, 
        Y.KaazingView,
        [],
        {
            ...
            updateClusterKeystores: function() {
                var clusterSecurityConfigs = this.buildKeystoreList();  // array of arrays
             ...
             * array of arrays of 'equal' keystores across all the gateways.
             */
            buildKeystoreList: function() {
                ...
                if (gatewayModels && gatewayModels.length > 0) {
                    for (i = 0; i < gatewayModels.length; i++) {
                        ...
                        // see if the keystore info matches any other securityconfig
                        var found = false;

                        for (j = 0; j < securityConfigLists.length; j++) {
                            var securityConfigList = securityConfigLists[j];
                            if (securityConfig.equalsKeystoreInfo(securityConfigList[0])) {
                                found = true;
            ...
            /**
             * Render the data fields for all cluster-level keystores from
            ...
            renderData: function() {
                ...
                var dataDiv = container.one('#configSecurityKeystoreData');
                ...
                if (!clusterSecurityConfigs || clusterSecurityConfigs.length == 0) {
                    // XXX put up some message about there being no keystores declared?
                    return;
                }
                ...
                for (var i = 0; i < clusterSecurityConfigs.length; i++) {
                    var keystoreDiv = createDIV(content, 'configSecuritySection');

                    var securityConfigArray = clusterSecurityConfigs[i];

                    var securityConfig = securityConfigArray[0];

                    var header = createH(keystoreDiv, 3)
                        .setHTML('Keystore');

                    // The basic details
                    var table = createTABLE(keystoreDiv);

                    // The gateways the keystore is defined in
                    ...
                    var keystoreCertificateInfo = securityConfig.get('keystoreCertificateInfo');

                    // keystoreCertificateInfo is an array of objects, each with
                    // alias: (certificate alias)
                    // options (object of name/value pairs)
                    if (keystoreCertificateInfo && keystoreCertificateInfo.length > 0) {
                        ...
                        var tempCerts = keystoreCertificateInfo.map(function(elt, index, array) {
